...
const dataItemOptions = {
    tags: [
        ...
        {name: 'App-Name', value: 'Harlequin-Remote-Signing'},
    ]
}
...
/**
 * Get Remote Signing version from package.json or environment
 */
function getVersion(): string {
  let version = process.env.REMOTE_SIGNING_VERSION;

  // If no REMOTE_SIGNING_VERSION, try to read from package.json (updated by Nx Release)
  if (!version) {
    try {
  ...
  if (!version) {
    version = process.env.GITHUB_REF_NAME?.replace('remote-signing-v', '') || '0.0.0-dev';
  }

  console.log(chalk.blue(`🔐 Deploying Remote Signing Server version: ${version}`));
  return version;
}
...
function findBinaries(): string[] {
  ...
  try {
    ...
    const binaries = files.filter(file => {
      ...
      return (fileName === 'harlequin-remote-signing' || fileName === 'harlequin-remote-signing.exe') &&
             (ext === '' || ext === '.exe');
    });
...
function parsePlatformArch(filename: string): PlatformArch {
  const patterns = [
    // GoReleaser directory format (windows): harlequin-remote-signing_windows_amd64_v1/harlequin-remote-signing.exe
    /harlequin-remote-signing_([^_]+)_([^_]+)_v[\d.]+[\/\\]harlequin-remote-signing\.exe$/,
    // GoReleaser directory format: harlequin-remote-signing_darwin_amd64_v1/harlequin-remote-signing
    /harlequin-remote-signing_([^_]+)_([^_]+)_v[\d.]+[\/\\]harlequin-remote-signing$/,
    // Archive format: harlequin-remote-signing_1.2.3_linux_amd64.tar.gz
    /harlequin-remote-signing_[\d.-]+_([^_]+)_([^.]+)\.tar\.gz$/,
    // Binary format: harlequin-remote-signing-linux-amd64
...
async function createManifest(turboClient: TurboAuthenticatedClient, uploads: {uploadResponse: TurboUploadDataItemResponse, name: string}[], installScript: TurboUploadDataItemResponse | null): Promise<ManifestResult> {
  ...
  try {
    ...
    const manifest: ArweaveManifest = {
      ...
      version: '0.1.0',
      index: {
        path: 'install_remote_signing.sh'
      },
    ...
    // Add install script
    if (installScript) {
      manifest.paths['install_remote_signing.sh'] = {
        id: installScript.id
      };
    }
    ...
    const releasesData: ReleasesData = {
      releases: [
        {
          tag_name: `remote-signing-v${version}`,
          version: version,
...
async function main(): Promise<void> {
  try {
    const title = CONFIG.dryRun ? '🔐 Harlequin Remote Signing Arweave Deployment (DRY RUN)' : '🔐 Harlequin Remote Signing Arweave Deployment';
    ...
    const installScriptPath = join(__dirname, 'install_remote_signing.sh');
